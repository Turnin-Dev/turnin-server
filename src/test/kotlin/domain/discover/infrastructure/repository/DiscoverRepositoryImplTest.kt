...
class DiscoverRepositoryImplTest {
    ...
    @Test
    fun `fetchSharedUserKeywords - 비활성화 사용자는 조회되지 않는다`() = runTest {
        // given: 데이터 세팅
        val fakeVector = TestVectorFixture.unitVector(1.0f)
        val (user1, user2) = dbRule.dbQuery {
            // 사용자 생성
        ...
        // 전부 사용자 비활성화 설정
        setUserInactiveForTest(UserId(user1.value))
        setUserInactiveForTest(UserId(user2.value))
    ...
    @Test
    fun `fetchSharedUserKeywords - 비활성화 사용자 키워드는 조회되지 않는다`() = runTest {
        // given: 데이터 세팅
        val fakeVector = TestVectorFixture.unitVector(1.0f)
        val (user1, user2) = dbRule.dbQuery {
            // 사용자 생성
        ...
        // user1의 사용자 키워드만 전부 비활성화 설정
        setUserKeywordInactiveForTest(UserKeywordId(1L))
        setUserKeywordInactiveForTest(UserKeywordId(2L))

        // when
        val result = repository.fetchSharedUserKeywords(targetIds)

        // then: user1의 사용자 키워드를 제외하고 user2의 사용자 키워드 1개만 조회되어야 한다.
