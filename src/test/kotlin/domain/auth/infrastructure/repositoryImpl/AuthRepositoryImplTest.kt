...
class AuthRepositoryImplTest {
    ...
    @Test
    fun `save & findByProviderAndProviderId 성공 테스트`() = runBlocking {
        TestDatabaseFactory.init()
    ...
    @Test
    fun `findByProviderAndProviderId 실패 테스트 - 존재하지 않는 사용자`() = runBlocking {
    ...
    @Test
    fun `save 실패 테스트 - 중복된 providerId 저장 시도`() = runBlocking {
        val savedUser = repository.save(MockAuthUser)
